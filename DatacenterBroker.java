...
/**BEGINNING OF SUFFRAGE ALGORITHM**/
	...
	public void bindCloutletToVmsSuffrage() {
		...
		do {
			...
			cloudletList.get(targetCloudletId).setVmId(vmList.get(colIndexOfMin).getId());
	...
	private static Double calculateSuffrage(Double[] arr) {
		...
		if (second == Double.MAX_VALUE)
			...
			System.out.printf("The smallest is %.4f and second smallest is %.4f \n", first, second);
		Double suffrage = second - first;
	...
	private void assignAndUpdate(List<Double[]> vmSuffrage, Double[] readyTime, List<List<Double>> tasksVmsMatrix, List<? extends Cloudlet> cloudletList,List<? extends Vm> vmList) {
